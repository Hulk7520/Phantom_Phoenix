...
#define BalanceDivFactor 5    //;Other values than 6 can be used, testing...CAUTION!! At your own risk ;)
#include <Wire.h>
#include "BioloidEx.h"
//#include <I2CEEProm.h>
...
boolean        fRobotWasUpsideDown; // Get some space between the readings
//=============================================================================
// Function prototypes
//=============================================================================
...
void loop(void)
{
  ...
  WriteOutputs();        // Write Outputs
    ...
    if (!fWalking){// dont do this while walking
      fRobotUpsideDown = fRobotWasUpsideDown; //Grab the state of the robot for a little while ago... 
...
#ifdef IsRobotUpsideDown
  if (!fWalking){// dont do this while walking
      fRobotWasUpsideDown = IsRobotUpsideDown;    // Grab the current state of the robot... 
  }
#endif
...
void SingleLegControl(void)
{
  ...
  if (g_InControlState.SelectedLeg<=5) {
    if (g_InControlState.SelectedLeg!=PrevSelectedLeg) {
      ...
      if (g_InControlState.SelectedLeg>2)   //Invert X direction for the left legs
